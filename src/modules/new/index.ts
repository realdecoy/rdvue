...
const availableFeaturesWithNoNames: string[] = Object.values(
  featuresWithNoNames
);
...
async function run(operation: Command, USAGE: CLI): Promise<any> {
  try {
    ...
    if (
      !util.isOptionalModuleAction(userAction) &&
      availablePlugins.includes(userFeature)
    ) {
    ...
    if (currentConfig.routes !== undefined) {
      await util.parseDynamicObjects(
        JSON.stringify(currentConfig.routes, null, 1),
        DYNAMIC_OBJECTS.routes
      );
    }
    ...
    if (currentConfig.stores !== undefined) {
      await util.parseDynamicObjects(
        JSON.stringify(currentConfig.stores, null, 1),
        DYNAMIC_OBJECTS.stores
      );
    }
    ...
    if (currentConfig.vueOptions !== undefined) {
      await util.parseDynamicObjects(
        JSON.stringify(currentConfig.vueOptions, null, 1),
        DYNAMIC_OBJECTS.options,
        true
      );
    }
    ...
    if (currentConfig.modules !== undefined) {
      await util.parseDynamicObjects(
        JSON.stringify(currentConfig.modules, null, 1),
        DYNAMIC_OBJECTS.modules,
        true
      );
    }

    // Update the .rdvue/modules.js file in src directory in the project
    if (currentConfig.projectModules !== undefined) {
      await util.updateDynamicImportsAndExports(
        DYNAMIC_OBJECTS.modules,
        currentConfig.projectModules
      );
    }
    ...
    if (currentConfig.packages !== undefined) {
      ...
      if (currentConfig.packages?.dependencies?.length > 0) {
        config.save = true;
        await util.dependencyInstaller(
          currentConfig.packages.dependencies,
          config
        );
      }
      ...
      if (currentConfig.packages?.devDependencies?.length > 0) {
        ...
        config.saveDev = true;
        await util.dependencyInstaller(
          currentConfig.packages.devDependencies,
          config
        );
      }
    }
