 ...
 * and returns an array of all selected modules
 * @param featureGroups
 */
    ...
    for (const group of featureGroups) {
      ...
      if (isArray(feature)) {
        feature.forEach((name) => {
          if (util.isOptionalFeature(name)) { selectedmodules.push(name); }
        });
      } else {
        if (util.isOptionalFeature(feature)) {

          selectedmodules.push(feature as string);
        }
      }
    }
  }
...
async function handleFeatureGroupsQuestions2(featureGroups?: Group[]) {
  let selectedmodules = new Array<string>();
  ...
  if (featureGroups !== undefined) {
    for (const group of featureGroups) {
      ...
      const feature = await inquirer.prompt(CONFIG.getQuestionByGroup(group)) as ModuleName[];

      const names = feature.filter(({ name }) => util.isOptionalFeature(name));
    ...
    try {
      ...
      if (isProject) {
        ...
        if (isPresetSelected(selectedPreset)) {
          ...
          modulesToInstall = await handleFeatureGroupsQuestions2(loadFeatureGroups());
        }
        ...
        // Load in optional features after project has been setup
