 ...
 * @param value - a string value
 */
 const validateProjectName = function(value: any) {
 ...
 * @param value - a string value
 */
 const validatePageName = function(value: any) {
  const isString = typeof value === 'string';
  const isNull = value === null || value.length === 0;
  // characters in value are limited to alphanumeric characters and hyphens or underscores
  const charactersMatch = value.match(/^[a-zA-Z0-9.\-_]+$/i) !== null; 
 ...
 * @param value - a string value
 */
const parseProjectName = async function(args: Lookup): Promise<string> {
 ...
 * @param value - a string value
 */
 const parsePageName = async function(args: Lookup): Promise<string> {
  let projectName = args['name'];
